...
Previous Go backend retained for reference. The active backend is in
`backend/`; this archived module remains independently buildable.
...
- `DEEPGRAM_API_KEY` in `backend-old/.env` for local transcription
...
```bash
./backend-old/build-and-push-image.sh
```
