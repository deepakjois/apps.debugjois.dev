...
import (
	...
	"github.com/deepakjois/apps.debugjois.dev/backend-old/internal/podcastaddict"
	"github.com/deepakjois/apps.debugjois.dev/backend-old/internal/transcribe"
...
func run(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer, client *http.Client) error {
	...
	if options.write {
		if err := persistTranscriptStoreFunc(ctx, "process-podcast-transcription", result, body); err != nil {
			return err
		}
	}
...
func parseArgs(args []string, stdin io.Reader) (cliOptions, string, error) {
	flags := flag.NewFlagSet("transcribe-podcast", flag.ContinueOnError)
