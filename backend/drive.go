...
func loadDailyLogContentFromDrive(ctx context.Context, date string) (string, error) {
...
func saveDailyLogContentToDrive(ctx context.Context, title, contents string) error {
	srv, did, folderID, err := getDriveClient(ctx)
	if err != nil {
		return err
	}
