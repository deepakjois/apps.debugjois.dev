...
import { formatTranscriptDate } from "./data";
import type { TranscriptIndexItem } from "./data";
...
export default function TranscriptSearchModal({
  ...
  useEffect(() => {
    // Global shortcuts mirror the transcript search trigger.
  ...
  function handleSearchKeyDown(event: ReactKeyboardEvent<HTMLInputElement>) {
    // Arrow keys change the active result without moving focus out of the search field.
    ...
    if (event.key === "Enter") {
      event.preventDefault();
      const selectedItem = visibleResults[clampedActiveResultIndex];
  ...
  // Native option values use transcript locations, which are unique within the index.
  const clampedActiveResultIndex = Math.min(
    activeResultIndex,
    Math.max(visibleResults.length - 1, 0),
  );
  const activeResultValue = visibleResults[clampedActiveResultIndex]?.location ?? "";
